...
public class TrainingServlet extends HttpServlet {
    @EJB
    TrainingEjb recordEjb;

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        RequestDispatcher rd;
        String coachPhone = req.getParameter("coachPhone");
        String userPhone = req.getSession().getAttribute(ParamsConstants.USER).toString();

        Training training = recordEjb.getUserTrainingByCoach(coachPhone, userPhone);
